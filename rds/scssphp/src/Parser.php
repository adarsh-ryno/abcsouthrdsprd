...
class Parser
{
	...
	protected function parseChunk()
	{
		...
		if (
			...
			if (
				...
				$this->logger->warn(
					"The \"@scssphp-import-once\" directive is deprecated and will be removed in ScssPhp 2.0, in \"$file\", line $line, column $column.",
					true
				);
			...
			if (
				...
				if ($this->cssOnly) {
					$this->assertPlainCssValid(
						[Type::T_IMPORT, $importPath],
						$s
					);
					$this->append([
						Type::T_COMMENT,
						rtrim(substr($this->buffer, $s, $this->count - $s)),
					]);
					return true;
				}
			...
			if (
				...
				$this->append(
					[Type::T_RETURN, isset($retVal) ? $retVal : [Type::T_NULL]],
					$s
				);
			...
			if (
				...
				$this->append(
					[
						Type::T_MIXIN_CONTENT,
						isset($argContent) ? $argContent : null,
					],
					$s
				);
			...
			if (isset($last) && $last[0] === Type::T_IF) {
				...
				if ($this->literal("@else", 5)) {
					if ($this->matchChar("{", false)) {
						...
						$this->literal("if", 2) &&
						$this->functionCallArgumentsList(
							$cond,
							false,
							"{",
							false
			...
			if (
				...
				if (
					...
					$this->env->type &&
					!\in_array($this->env->type, [
						Type::T_DIRECTIVE,
						Type::T_MEDIA,
					])
				) {
					$plain = \trim(
						\substr($this->buffer, $s, $this->count - $s)
					);
					throw $this->parseError(
						"Unknown directive `{$plain}` not allowed in `" .
							$this->env->type .
							"` block"
					);
				}
				...
				$this->append(
					[
						Type::T_DIRECTIVE,
						[$dirName, $dirValue, $hasBlankLine, $isNotRoot],
					],
					$s
				);
				$this->whitespace();
		...
		if (
			...
			$this->append(
				[Type::T_ASSIGN, $name, $value, $assignmentFlags],
				$s
			);
	...
	protected function assertPlainCssValid($parsed, $startPos = null)
	{
		...
		if (!$parsed) {
			if (!\is_null($startPos)) {
				$plain = rtrim(
					substr($this->buffer, $startPos, $this->count - $startPos)
				);
		...
		switch ($parsed[0]) {
			...
			case Type::T_DIRECTIVE:
				if (\is_array($parsed[1])) {
					$parsed[1][1] = $this->isPlainCssValidElement(
						$parsed[1][1]
					);
					if (!$parsed[1][1]) {
						return false;
			...
			case Type::T_LIST:
				...
				foreach ($parsed[2] as $k => $listElement) {
					$parsed[2][$k] = $this->isPlainCssValidElement(
						$listElement
					);
					if (!$parsed[2][$k]) {
						return false;
			...
			case Type::T_ASSIGN:
				foreach ([1, 2, 3] as $k) {
					if (!empty($parsed[$k])) {
						$parsed[$k] = $this->isPlainCssValidElement(
							$parsed[$k]
						);
						if (!$parsed[$k]) {
							return false;
			...
			case Type::T_FUNCTION_CALL:
				...
				foreach ($parsed[2] as $arg) {
					...
					$arg = $this->isPlainCssValidElement(
						$arg[1],
						$parsed[1] === "calc"
					);
					if (!$arg) {
	...
	{
		if (
			strcasecmp(substr($this->buffer, $this->count, $len), $what) !== 0
		) {
			return false;
		}
			...
			if (isset($m[1]) && empty($this->commentsSeen[$this->count])) {
				...
				while ($p !== false && $p < $endCommentCount) {
					...
					if ($this->interpolation($out)) {
						...
						$comment[] = [
							Type::T_COMMENT,
							substr($this->buffer, $p, $this->count - $p),
							$out,
						];
					} else {
						list($line, $column) = $this->getSourcePosition(
							$this->count
						);
						$file = $this->sourceName;
						if (!$this->discardComments) {
							$this->logger->warn(
								"Unterminated interpolations in multiline comments are deprecated and will be removed in ScssPhp 2.0, in \"$file\", line $line, column $column.",
								true
							);
						}
				...
				if (!$comment) {
					...
					$comment[] = $c;
					$staticComment = substr(
						$this->buffer,
						$startCommentCount,
						$endCommentCount - $startCommentCount
					);
					$commentStatement = [
						Type::T_COMMENT,
						$staticComment,
	...
	protected function supportsQuery(&$out)
	{
		...
		if (
			...
			$parts[] = [
				Type::T_STRING,
				"",
				[[Type::T_KEYWORD, "("], $subQuery, [Type::T_KEYWORD, ")"]],
			];
			$s = $this->count;
		} else {
			$this->seek($s);
	 ...
	 * @return bool
	 */
	protected function functionCallArgumentsList(
		&$out,
		$mandatoryEnclos = true,
		$charAfter = null,
		$eatWhiteSp = null
	) {
		$s = $this->count;
		...
		if (
			...
			$this->matchChar(")") &&
			($charAfter
				? $this->matchChar($charAfter, $eatWhiteSp)
				: $this->end())
		) {
			return true;
		}
	 ...
	 * @return bool
	 */
	protected function genericList(
		&$out,
		$parseItem,
		$delim = "",
		$flatten = true
			...
			if ($delim) {
				...
				if ($value[0] === Type::T_KEYWORD) {
					...
					if (
						...
						while (
							substr($this->buffer, $this->count, 1) !==
							$last_char
						) {
							$this->count--;
						}
	...
	protected function expression(&$out, $listOnly = false, $lookForExp = true)
	{
		...
		$this->discardComments = true;
		$allowedTypes = $listOnly
			? [Type::T_LIST]
			: [Type::T_LIST, Type::T_MAP];
	 ...
	 */
	protected function enclosedExpression(
		&$out,
		$s,
		$closingParen = ")",
	...
	protected function expHelper($lhs, $minP)
	{
		...
		$ss = $this->count;
		$whiteBefore =
			isset($this->buffer[$this->count - 1]) &&
			ctype_space($this->buffer[$this->count - 1]);

		while (
			$this->match($operators, $m, false) &&
	...
	protected function interpolation(&$out, $lookWhite = true)
	{
		...
		if (
			...
			if ($value === [Type::T_SELF]) {
				...
				if ($lookWhite) {
					$left =
						$s > 0 && preg_match("/\s/", $this->buffer[$s - 1])
							? " "
							: "";
					$right =
	...
	protected function selector(&$out, $subSelector = false)
	{
		...
		for (;;) {
			...
			if ($this->match("[>+~]+", $m, true)) {
				if (
					$subSelector &&
					\is_string($subSelector) &&
					strpos($subSelector, "nth-") === 0 &&
					$m[0] === "+" &&
					$this->match("(\d+|n\b)", $counter)
				) {
					$this->seek($s);
				} else {
					$selector[] = [$m[0]];
	 ...
	 * @return bool
	 */
	protected function matchEscapeCharacterInSelector(
		&$out,
		$keepEscapedNumber = false
	) {
		$s_escape = $this->count;
		if ($this->match("\\\\", $m)) {
			$out = "\\" . $m[0];
		...
		if ($this->matchEscapeCharacter($escapedout, true)) {
			...
			$escape_sequence = rtrim(
				substr($this->buffer, $s_escape, $this->count - $s_escape)
			);
	...
	protected function selectorSingle(&$out, $subSelector = false)
	{
		...
		for (;;) {
			...
			if ($char === ":") {
				...
				if ($this->mixedKeyword($nameParts, true)) {
					...
					if (
						...
						if (
							$this->matchChar("(", true) &&
							($this->selectors($subs, reset($nameParts)) ||
								true) &&
							$this->matchChar(")")
						) {
							$parts[] = "(";
			...
			if (
				...
				if (
					$this->match("(\s*(\+\s*|\-\s*)?(\d+|n|\d+n))+", $counter)
				) {
					$parts[] = $counter[0];
	 ...
	 * @return bool
	 */
	protected function restrictedKeyword(
		&$word,
		$eatWhitespace = null,
		$inSelector = false
	) {
		...
		if (
			$this->keyword($word, $eatWhitespace, $inSelector) &&
	...
	protected function stripAssignmentFlags(&$value)
	{
		...
		for (
			$token = &$value;
			$token[0] === Type::T_LIST && ($s = \count($token[2]));
			$token = &$lastNode
		) {
			$lastNode = &$token[2][$s - 1];

			while (
				$lastNode[0] === Type::T_KEYWORD &&
	...
	private function saveEncoding()
	{
		if (
			...
			(2 & (int) ini_get("mbstring.func_overload")) > 0
		) {
