...
class Nested extends Formatter
{
	...
	{
		@trigger_error(
			"The Nested formatter is deprecated since 1.4.0. Use the Expanded formatter instead.",
			E_USER_DEPRECATED
		);
	...
	protected function block(OutputBlock $block)
	{
		...
		$isMediaOrDirective = \in_array($block->type, [
			Type::T_DIRECTIVE,
			Type::T_MEDIA,
		]);
		$isSupport =
		...
		while (
			$block->depth < end($depths) ||
			($block->depth == 1 && end($depths) == 1)
		) {
			array_pop($depths);
			$this->depth--;
			...
			if (
				...
				($block->depth <= 1 ||
					(!$this->indentLevel &&
						$block->type === Type::T_COMMENT)) &&
				(($block->selectors && !$isMediaOrDirective) ||
					$previousHasSelector)
			) {
		...
		if (
			!empty($block->lines) ||
			(!empty($block->children) && ($this->depth < 1 || $isSupport))
		) {
		...
		if (!empty($block->children)) {
			if (
				$this->depth > 0 &&
				($isMediaOrDirective || !$this->hasFlatChild($block))
			) {
				array_pop($depths);
