...
class Compiler
{
	...
	{
		@trigger_error(
			'The method "setErrorOuput" is deprecated. Use "setLogger" instead.',
			E_USER_DEPRECATED
		);
	...
	public function compile($code, $path = null)
	{
		@trigger_error(
			sprintf(
				'The "%s" method is deprecated. Use "compileString" instead.',
				__METHOD__
			),
			E_USER_DEPRECATED
		);
	...
	public function compileString($source, $path = null)
	{
		if ($this->cache) {
			...
			$cachedResult = $this->cache->getCache(
				"compile",
				$cacheKey,
				$compileOptions
			);
			...
			if (
				$cachedResult instanceof CachedResult &&
		...
		try {
				...
				switch ($this->sourceMap) {
					...
					case self::SOURCE_MAP_FILE:
						if (isset($this->sourceMapOptions["sourceMapURL"])) {
							$sourceMapUrl =
								$this->sourceMapOptions["sourceMapURL"];
						}
						break;
				}
			...
			throw new CompilerException(
				$this->addLocationToMessage($e->getMessage()),
				0,
				$e
			);
		}
		...
		foreach ($this->resolvedImports as $resolvedImport) {
			$includedFiles[$resolvedImport["filePath"]] =
		...
		if ($this->cacheCheckImportResolutions) {
			...
			foreach ($result->getResolvedImports() as $import) {
				...
				if (empty($resolvedImports[$currentDir][$path])) {
					$resolvedImports[$currentDir][$path] = $this->findImport(
						$path,
						$currentDir
					);
				}
				...
				if (
					$resolvedImports[$currentDir][$path] !== $import["filePath"]
				) {
					return false;
				}
			}
	...
	protected function makeOutputBlock($type, $selectors = null)
	{
		...
		if ($this->env->block instanceof Block) {
			...
			$out->sourceName = isset($this->sourceNames[$this->sourceIndex])
	...
	protected function missingSelectors()
	{
		foreach ($this->extends as $extend) {
			...
			throw $this->error(
				"\"$origin\" failed to @extend \"$target\". The selector \"$target\" was not found."
			);
		}
	}
	 ...
	 * @return void
	 */
	protected function matchExtends(
		$selector,
		&$out,
		$from = 0,
		$initial = true
			...
			if ($this->matchExtendsSingle($part, $origin, $initial)) {
				...
				foreach ($origin as $new) {
					...
					for ($l = \count($tempReplacement) - 1; $l >= 0; $l--) {
						...
						if (
							!$this->isImmediateRelationshipCombinator(
								end($slice)
							)
						) {
							break;
					...
					$afterBefore =
						$l != 0 ? \array_slice($tempReplacement, 0, $l) : [];

					// Merge shared direct relationships.
					$mergedBefore = $this->mergeDirectRelationships(
						$afterBefore,
						$nonBreakableBefore
					);
					...
					if (!empty($before) && \count($new) > 1) {
						...
						list(
							$betweenSharedParts,
							$nonBreakabl2,
						) = $this->extractRelationshipFromFragment(
							$afterBefore
						);
	 ...
	 * @return bool
	 */
	protected function matchExtendsSingle(
		$rawSingle,
		&$outOrigin,
		$initial = true
	) {
		...
		if (\count($single) > 1) {
			$matches = null;
			$extendingDecoratedTag = preg_match(
				'/^[a-z0-9]+$/i',
				$single[0],
				$matches
			)
					...
					foreach ($subSelectors as $ksub => $subSelector) {
						...
						if ($subExtended) {
							...
							foreach ($subSelectorsExtended as $ksse => $sse) {
								$subSelectorsExtended[
									$ksse
								] = $this->collapseSelectors($sse);
							}

							$subSelectorsExtended = implode(
								", ",
								$subSelectorsExtended
							);
		...
		foreach ($counts as $idx => $count) {
			...
			foreach ($origin as $j => $new) {
				...
				if (
					...
					$replacement[0] != $extendingDecoratedTag &&
	...
	protected function compileMedia(Block $media)
	{
		...
		$mediaQueries = $this->compileMediaQuery(
			$this->multiplyMedia($this->env)
		);
	...
	protected function compileDirective($directive, OutputBlock $out)
	{
		if (\is_array($directive)) {
			...
			if (empty($directive[3])) {
				$this->appendRootDirective($s . ";" . $appendNewLine, $out, [
					Type::T_COMMENT,
					Type::T_DIRECTIVE,
				]);
			} else {
			...
			if (
				$directive->name === "keyframes" ||
				substr($directive->name, -10) === "-keyframes"
			) {
	...
	protected function compileAtRoot(Block $block)
	{
		...
		list($with, $without) = $this->compileWith(
			isset($block->with) ? $block->with : null
		);
		...
		assert(
			$selfParent !== null,
			"at-root blocks must have a selfParent set."
		);
		...
		$this->scope = $this->filterScopeWithWithout(
			$saveScope,
			$with,
			$without
		);
		...
		if ($withCondition) {
			...
			if ($withConfig !== null) {
				...
				foreach ($list[2] as $item) {
					$keyword = $this->compileStringContent(
						$this->coerceString($item)
					);
	...
	protected function isWith($block, $with, $without)
	{
		if (isset($block->type)) {
			...
			if ($block->type === Type::T_DIRECTIVE) {
				assert(
					$block instanceof DirectiveBlock ||
						$block instanceof OutputBlock
				);
				if (isset($block->name)) {
					return $this->testWithWithout(
	...
	protected function testWithWithout($what, $with, $without)
	{
		...
		if (\count($without)) {
			return isset($without[$what]) || isset($without["all"])
				? false
				: true;
		}
	...
	protected function compileKeyframeBlock(Block $block, $selectors)
	{
		...
		$this->env = $this->extractEnv(
			array_filter($envs, function (Environment $e) {
	...
	protected function compileNestedBlock(Block $block, $selectors)
	{
		...
		if (
			!$block instanceof DirectiveBlock ||
			$this->compileDirectiveName($block->name) !== "font-face"
		) {
			// need wrapping?
			$needWrapping = false;
	...
	protected function compileBlock(Block $block)
	{
		...
		if (\count($block->children)) {
			$out->selectors = $this->multiplySelectors(
				$env,
				$block->selfParent
			);
	...
	protected function compileCommentValue($value, $pushEnv = false)
	{
		...
		if (isset($value[2])) {
			...
			try {
				...
				$this->logger->warn(
					"Ignoring interpolation errors in multiline comments is deprecated and will be removed in ScssPhp 2.0. " .
						$this->addLocationToMessage($e->getMessage()),
					true
				);
	...
	protected function evalSelectorPart($part)
	{
		foreach ($part as &$p) {
			if (
				\is_array($p) &&
				($p[0] === Type::T_INTERPOLATE || $p[0] === Type::T_STRING)
			) {
	...
	protected function collapseSelectors($selectors)
	{
		...
		foreach ($selectors as $selector) {
			...
			foreach ($selector as $node) {
				...
				array_walk_recursive($node, function ($value, $key) use (
					&$compound
				) {
					$compound .= $value;
				});
	...
	protected function compileSelector($selector)
	{
		...
		return implode(
			" ",
			array_map([$this, "compileSelectorPart"], $selector)
		);
	}
	 ...
	 * @throws \Exception
	 */
	protected function compileChildrenNoReturn(
		$stms,
		OutputBlock $out,
		$selfParent = null,
		$traceName = ""
		...
		foreach ($stms as $stm) {
			if (
				...
				$selfParent &&
				\in_array($stm[0], [Type::T_INCLUDE, Type::T_EXTEND])
			) {
			...
			foreach ($query as $q) {
				switch ($q[0]) {
					case Type::T_MEDIA_TYPE:
						...
						if (!$mediaTypeOnly) {
							if (
								\in_array(Type::T_NOT, $newType) ||
								($type && \in_array(Type::T_NOT, $type))
							) {
								if ($type) {
									array_unshift(
										$parts,
										implode(" ", array_filter($type))
									);
								}
	...
	protected function mergeDirectRelationships($selectors1, $selectors2)
	{
		...
		if (
			!$this->isImmediateRelationshipCombinator($part1[0]) &&
			$part1 !== $part2
		) {
			return array_merge($selectors1, $selectors2);
		}
	...
	protected function compileImport($rawPath, OutputBlock $out, $once = false)
	{
		if ($rawPath[0] === Type::T_STRING) {
			...
			if (
				strpos($path, "url(") !== 0 &&
				($filePath = $this->findImport($path, $this->currentDirectory))
			) {
				$this->registerImport(
			...
			$this->appendRootDirective(
				"@import " . $this->compileImportPath($rawPath) . ";",
				$out
			);
	 ...
	 * @return void
	 */
	protected function appendRootDirective(
		$line,
		$out,
		...
		while ($i < \count($root->children)) {
			if (
				!isset($root->children[$i]->type) ||
				!\in_array($root->children[$i]->type, $allowed)
			) {
				break;
			}
		...
		$child->lines[] = $line;
		$child->sourceName =
			$this->sourceNames[$this->sourceIndex] ?: "(stdin)";
		...
		switch ($child[0]) {
			...
			case Type::T_ASSIGN:
				...
				if (
					...
					if (
						$shorthandValue[0] === Type::T_EXPRESSION &&
						$shorthandValue[1] === "/"
					) {
						$revert = true;
						...
						if ($shorthandDividerNeedsUnit) {
							...
							if (
								$divider instanceof Number &&
								\intval($divider->getDimension()) &&
								$divider->unitless()
							) {
								$revert = false;
							}
						...
						if ($revert) {
							$shorthandValue = $this->expToString(
								$shorthandValue
							);
						}
			...
			case Type::T_FUNCTION:
				...
				$this->set(
					static::$namespaces[$block->type] . $block->name,
					$block,
					true
				);
				break;
...
EOL;
			...
			case Type::T_FOR:
				...
				$startNumber = $this->assertNumber(
					$this->reduce($for->start, true)
				);
				$endNumber = $this->assertNumber(
					$this->reduce($for->end, true)
				);
				...
				$end = $this->assertInteger(
					$endNumber->coerce($numeratorUnits, $denominatorUnits)
				);
				...
				for (;;) {
					...
					$this->set(
						$for->var,
						new Number($start, $numeratorUnits, $denominatorUnits)
					);
					$start += $d;
			...
			case Type::T_INCLUDE:
				...
				if (
					isset($child["selfParent"]) &&
					$child["selfParent"] instanceof Block &&
				...
				if (isset($content)) {
					...
					$this->setRaw(
						static::$namespaces["special"] . "content",
						$copyContent,
						$this->env
					);
				} else {
				...
				if (isset($argUsing)) {
					$this->setRaw(
						static::$namespaces["special"] . "using",
						$argUsing,
						$this->env
					);
				} else {
		...
		switch ($value[0]) {
			case Type::T_EXPRESSION:
				...
				if (
					...
					\is_callable([$this, ($fn = "op{$ucLType}{$ucRType}")])
				) {
			...
			case Type::T_SELF:
				$selfParent = !empty($this->env->block->selfParent)
					? $this->env->block->selfParent
					: null;
				$selfSelector = $this->multiplySelectors(
					$this->env,
					$selfParent
				);
	...
	protected function fncall($functionReference, $argValues)
	{
		...
		if (is_string($functionReference)) {
			...
			if (
				$functionReference === static::$null ||
				$functionReference[0] !== Type::T_FUNCTION_REFERENCE
			) {
				$functionReference = [
					Type::T_FUNCTION,
					$name,
		...
		if ($functionReference[0] === Type::T_FUNCTION) {
			...
			foreach ((array) $argValues as $arg) {
				if (empty($arg[0]) || count($argValues) === 1) {
					$listArgs[] = $this->reduce(
						$this->stringifyFncallArgs($arg[1])
					);
				}
			}
		...
		switch ($functionReference[1]) {
			...
			case "native":
				...
				$returnValue = $this->callNativeFunction(
					$name,
					$fn,
					$prototype,
					$argValues
				...
				if (!isset($returnValue)) {
					return $this->fncall(
						[Type::T_FUNCTION, $name, [Type::T_LIST, ",", []]],
						$argValues
					);
				}
	 ...
	 */
	protected function cssValidArg(
		$arg,
		$allowed_function = [],
		$inFunction = false
	) {
		...
		switch ($arg[0]) {
			...
			case Type::T_FUNCTION_CALL:
				...
				return $this->fncall(
					[Type::T_FUNCTION, $arg[1], [Type::T_LIST, ",", []]],
					$cssArgs
				);
			...
			case Type::T_EXPRESSION:
				...
				$arg[2] = $this->cssValidArg(
					$arg[2],
					$allowed_function,
					$inFunction
				);
				$arg[3] = $this->cssValidArg(
					$arg[3],
					$allowed_function,
					$inFunction
				);
	...
	protected function getFunctionReference($name, $safeCopy = false)
	{
		...
		if (
			($f = $this->getBuiltinFunction($normalizedName)) &&
			\is_callable($f)
		) {
			/** @var string $libName */
			$libName = $f[1];
			...
			if (
				...
				if (
					...
					$kebabCaseName = preg_replace(
						"~(?<=\\w)([A-Z])~",
						'-$1',
						substr($actualLibName, 3)
					);
					...
					$prototype = isset(static::$$actualLibName)
						? static::$$actualLibName
						: null;
			...
			if (
				...
				!isset($this->warnedChildFunctions[$libName])
			) {
				$r = new \ReflectionMethod($this, $libName);
				$declaringClass = $r->getDeclaringClass()->name;

				$needsWarning = $this->warnedChildFunctions[$libName] =
					$declaringClass !== __CLASS__;

				if ($needsWarning) {
					if (method_exists(__CLASS__, $libName)) {
						@trigger_error(
							sprintf(
								'Overriding the "%s" core function by extending the Compiler is deprecated and will be unsupported in 2.0. Remove the "%s::%s" method.',
								$normalizedName,
								$declaringClass,
								$libName
							),
						...
						@trigger_error(
							sprintf(
								'Registering custom functions by extending the Compiler and using the lib* discovery mechanism is deprecated and will be removed in 2.0. Replace the "%s::%s" method with registering the "%s" function through "Compiler::registerFunction".',
								$declaringClass,
								$libName,
								$normalizedName
							),
	...
	protected function opColorColor($op, $left, $right)
	{
		if ($op !== "==" && $op !== "!=") {
			$warning =
				"Color arithmetic is deprecated and will be an error in future versions.\n" .
	...
	protected function opColorNumber($op, $left, Number $right)
	{
		...
		return $this->opColorColor($op, $left, [
			Type::T_COLOR,
			$value,
			$value,
			$value,
	...
	{
		if (
			($lStr = $this->coerceString($left)) &&
			($rStr = $this->coerceString($right))
		) {
			$lStr[1] = "";
			$rStr[1] = "";
	...
	public function escapeNonPrintableChars($string, $inKeyword = false)
	{
		...
		if (empty($replacement[$inKeyword])) {
			for ($i = 0; $i < 32; $i++) {
				if ($i !== 9 || $inKeyword) {
					$replacement[$inKeyword][chr($i)] =
						"\\" . dechex($i) . ($inKeyword ? " " : chr(0));
				}
			}
		}
		$string = str_replace(
			array_keys($replacement[$inKeyword]),
			array_values($replacement[$inKeyword]),
			$string
		);
		...
		if (strpos($string, chr(0)) !== false) {
			...
			if (strpos($string, chr(0)) !== false) {
				...
				while (count($parts)) {
					...
					if (
						strpos("0123456789abcdefABCDEF" . chr(9), $next[0]) !==
						false
					) {
						$string .= " ";
					}
		...
		switch ($value[0]) {
			...
			case Type::T_COLOR:
				...
				if (\count($value) === 5) {
					...
					if (!is_numeric($alpha) || $alpha < 1) {
						$colorName = Colors::RGBaToColorName(
							$r,
							$g,
							$b,
							$alpha
			...
			case Type::T_STRING:
				...
				if ($value[1] && $quote) {
					...
					if (
						$value[1] === "'" &&
						(strpos($content, '"') === false or
							strpos($content, "'") !== false)
					) {
						$value[1] = '"';
					} elseif (
						$value[1] === '"' &&
						(strpos($content, '"') !== false and
							strpos($content, "'") === false)
					) {
						$value[1] = "'";
					}
			...
			case Type::T_MAP:
				...
				for ($i = 0, $s = \count($keys); $i < $s; $i++) {
					$filtered[
						$this->compileValue($keys[$i], $quote)
					] = $this->compileValue($values[$i], $quote);
				}
			...
			case Type::T_INTERPOLATED:
				...
				$right =
					\count($right[2]) > 0
						? $whiteRight .
							$delim .
							$this->compileValue($right, $quote)
						: "";

				return $left .
					$this->compileValue($interpolate, $quote) .
					$right;
				...
				switch ($reduced[0]) {
					case Type::T_LIST:
						...
						foreach ($items as $item) {
							...
							if ($item[0] === Type::T_STRING) {
								$filtered[] = $this->compileStringContent(
									$item,
									$quote
								);
			...
			default:
				throw $this->error(
					"unknown value type: " . json_encode($value)
				);
		}
	}
	...
	public function getStringText(array $value)
	{
		if ($value[0] !== Type::T_STRING) {
			throw new \InvalidArgumentException(
				'The argument is not a sass string. Did you forgot to use "assertString"?'
			);
		}
     ...
     * @return array
     */
	protected function joinSelectors(
		$parent,
		$child,
		&$stillHasSelf,
		$selfParentSelectors = null
	 ...
	 * @return array
	 */
	protected function multiplyMedia(
	...
	public function addVariables(array $variables)
	{
		...
		if ($triggerWarning) {
			@trigger_error(
				'Passing raw values to as custom variables to the Compiler is deprecated. Use "\ScssPhp\ScssPhp\ValueConverter::parseValue" or "\ScssPhp\ScssPhp\ValueConverter::fromPhp" to convert them instead.',
				E_USER_DEPRECATED
			);
		}
	}
	...
	{
		@trigger_error(
			'The method "getParsedFiles" of the Compiler is deprecated. Use the "getIncludedFiles" method on the CompilationResult instance returned by compileString() instead. Be careful that the signature of the method is different.',
			E_USER_DEPRECATED
		);
	...
	public function setImportPaths($path)
	{
		...
		$this->legacyCwdImportPath =
			\count($actualImportPaths) !== \count($paths);
	...
	public function setNumberPrecision($numberPrecision)
	{
		@trigger_error(
			"The number precision is not configurable anymore. " .
				"The default is enough for all browsers.",
			E_USER_DEPRECATED
		);
	}
	...
	public function setOutputStyle($style)
	{
		switch ($style) {
			...
			default:
				throw new \InvalidArgumentException(
					sprintf('Invalid output style "%s".', $style)
				);
		}
	}
	...
	public function setFormatter($formatterName)
	{
		if (
			!\in_array(
				$formatterName,
				[Expanded::class, Compressed::class],
				true
			)
		) {
			@trigger_error(
				"Formatters other than Expanded and Compressed are deprecated.",
				E_USER_DEPRECATED
			);
		}
		@trigger_error(
	...
	public function registerFunction(
		...
		if (self::isNativeFunction($name)) {
			@trigger_error(
				sprintf(
					'The "%s" function is a core sass function. Overriding it with a custom implementation through "%s" is deprecated and won\'t be supported in ScssPhp 2.0 anymore.',
					$name,
					__METHOD__
				),
				E_USER_DEPRECATED
		...
		$this->userFunctions[$this->normalizeName($name)] = [
			$callback,
			$argumentDeclaration,
		];
	}
			...
			foreach ($this->importPaths as $dir) {
				...
				if (\is_callable($dir)) {
					...
					if (!\is_null($file)) {
						...
						@trigger_error(
							sprintf(
								'Returning a file to import for CSS or external references in custom importer callables is deprecated and will not be supported anymore in ScssPhp 2.0. This behavior is not compliant with the Sass specification. Update your "%s" importer.',
								$callableDescription
							),
							E_USER_DEPRECATED
						);
	...
	private function resolveImportPath($url, $baseDir)
	{
		...
		$result = $this->checkImportPathConflicts(
			$this->tryImportPathWithExtensions($path)
		);
	...
	private function tryImportPathAsDirectory($path)
	{
		...
		return $this->checkImportPathConflicts(
			$this->tryImportPathWithExtensions($path . "/index")
		);
	}
	...
	private function getPrettyPath($path)
	{
		...
		if (\DIRECTORY_SEPARATOR === "\\") {
			$normalizedRootDirectory = str_replace(
				"\\",
				"/",
	...
	public function setEncoding($encoding)
	{
		if (!$encoding || strtolower($encoding) === "utf-8") {
			...
			@trigger_error(
				sprintf(
					'The "%s" method is deprecated. Parsing will only support UTF-8 in ScssPhp 2.0. The non-UTF-8 parsing of ScssPhp 1.x is not spec compliant.',
					__METHOD__
				),
				E_USER_DEPRECATED
			);
	...
	protected function callNativeFunction($name, $function, $prototype, $args)
	{
		...
		$sorted_kwargs = $this->sortNativeFunctionArgs(
			$libName,
			$prototype,
			$args
		);
		...
		@trigger_error(
			sprintf(
				'Returning a PHP value from the "%s" custom function is deprecated. A sass value must be returned instead.',
				$name
			),
			E_USER_DEPRECATED
		);
	...
	public static function isNativeFunction($name)
	{
		return method_exists(
			Compiler::class,
			self::normalizeNativeFunctionName($name)
		);
	}
	...
	protected function sortNativeFunctionArgs($functionName, $prototypes, $args)
	{
		...
		if (
			...
			foreach ($args as $k => $arg) {
				...
				if ($arg[1][0] === Type::T_LIST && \count($arg[1][2]) === 3) {
					$args[$k][1][2] = $this->extractSlashAlphaInColorFunction(
						$arg[1][2]
					);
				}
			}
		...
		list(
			...
			$separator,
			$hasSplat,
		) = $this->evaluateArguments($args, false);

		if (!\is_array(reset($prototypes))) {
			$prototypes = [$prototypes];
		}

		$parsedPrototypes = array_map(
			[$this, "parseFunctionPrototype"],
			$prototypes
		);
		assert(!empty($parsedPrototypes));
		$matchedPrototype = $this->selectFunctionPrototype(
			$parsedPrototypes,
		...
		$vars = $this->applyArgumentsToDeclaration(
			$matchedPrototype,
			$positionalArgs,
			$namedArgs,
			$separator
	...
	private function parseFunctionPrototype(array $prototype)
	{
		...
		foreach ($prototype as $p) {
			if (null !== $restArgument) {
				throw new \InvalidArgumentException(
					"The argument declaration is invalid. The rest argument must be the last one."
				);
			}
	 ...
	 * @phpstan-return array{arguments: list<array{0: string, 1: string, 2: array|Number|null}>, rest_argument: string|null}
	 */
	private function selectFunctionPrototype(
		array $prototypes,
		$positional,
		array $names
	) {
		...
		foreach ($prototypes as $prototype) {
			...
			if ($minMismatchDistance !== null) {
				...
				if (
					abs($mismatchDistance) === abs($minMismatchDistance) &&
					$mismatchDistance < 0
				) {
					continue;
				}
	 ...
	 */
	private function checkPrototypeMatches(
		array $prototype,
		$positional,
		array $names
	) {
	 ...
	 * @phpstan-param array{arguments: list<array{0: string, 1: string, 2: array|Number|null}>, rest_argument: string|null} $prototype
	 */
	private function verifyPrototype(
		array $prototype,
		$positional,
		array $names,
		$hasSplat
			...
			if ($i < $positional) {
				if (isset($names[$name])) {
					throw new SassScriptException(
						sprintf(
							'Argument $%s was passed both by position and by name.',
							$originalName
						)
					);
				}
				...
				throw new SassScriptException(
					sprintf('Missing argument $%s', $originalName)
				);
			}
		}
		...
		if ($nameUsed < \count($names)) {
			$unknownNames = array_values(
				array_diff($names, array_column($prototype["arguments"], 0))
			);
		...
		foreach ($args as $arg) {
			if (!empty($arg[0])) {
				...
				if (isset($keywordArgs[$name])) {
					throw new SassScriptException(
						sprintf('Duplicate named argument $%s.', $arg[0][1])
					);
				}
				...
				if ($val[0] === Type::T_LIST) {
					...
					if (isset($val[3]) && \is_array($val[3])) {
						foreach ($val[3] as $name => $item) {
							...
							$keywordArgs[$normalizedName] = $this->maybeReduce(
								$reduce,
								$item
							);
					...
					foreach ($val[1] as $i => $name) {
						$name = $this->compileStringContent(
							$this->coerceString($name)
						);
						$item = $val[2][$i];
						...
						if (!is_numeric($name)) {
							...
							$positionalArgs[] = $this->maybeReduce(
								$reduce,
								$item
							);
						}
					}
				} elseif ($val[0] !== Type::T_NULL) {
	...
	protected function applyArguments(
		...
		list(
			...
			$splatSeparator,
			$hasSplat,
		) = $this->evaluateArguments($argValues, $reduce);
	 ...
	 * @phpstan-param array{arguments: list<array{0: string, 1: string, 2: array|Number|null}>, rest_argument: string|null} $prototype
	 */
	private function applyArgumentsToDeclaration(
		array $prototype,
		...
		for (
			$i = \count($positionalArgs);
			$i < \count($prototype["arguments"]);
			$i++
		) {
		...
		if ($prototype["rest_argument"] !== null) {
			...
			$rest = array_values(
				array_slice($positionalArgs, \count($prototype["arguments"]))
			);

			$val = [
				Type::T_LIST,
				\is_null($splatSeparator) ? "," : $splatSeparator,
				$rest,
				$restNamed,
			];
		...
		if (!is_numeric($value)) {
			...
			if ($value instanceof Number) {
				if ($value->unitless()) {
					...
					throw $this->error(
						'Expected %s to have no units or "%%".',
						$value
					);
				}
	...
	public function assertString($value, $varName = null)
	{
		...
		if (!\in_array($value[0], [Type::T_STRING, Type::T_KEYWORD])) {
			$value = $this->compileValue($value);
			throw SassScriptException::forArgument(
	...
	public function getArgumentListKeywords($value)
	{
		if (
			...
			throw new \InvalidArgumentException(
				"The argument is not a sass argument list."
			);
		}
	...
	protected function libCall($args)
	{
		...
		if (
			in_array($functionReference[0], [Type::T_STRING, Type::T_KEYWORD])
		) {
			$name = $this->compileStringContent(
				$this->coerceString($functionReference)
			);
			$warning =
				"Passing a string to call() is deprecated and will be illegal\n" .
				"in Sass 4.0. Use call(function-reference($name)) instead.";
			Warn::deprecation($warning);
			$functionReference = $this->libGetFunction([
				$this->assertString($functionReference, "function"),
			]);
		}
		...
		if (
			!in_array($functionReference[0], [
				Type::T_FUNCTION_REFERENCE,
				Type::T_FUNCTION,
			])
		) {
			throw $this->error(
				"Function reference expected, got " . $functionReference[0]
			);
		}

		$callArgs = [[null, $args[1], true]];
	...
	{
		$name = $this->compileStringContent(
			$this->assertString(array_shift($args), "name")
		);
		$isCss = false;
	...
	protected function alterColor(array $args, $operation, $fn)
	{
		...
		$getParam = function (
			...
			if (!$scale && $checkPercent) {
				if (!$number->hasUnit("%")) {
					$warning = $this->error(
						"{$name} Passing a number `$number` without unit % is deprecated."
					);
		...
		if ($hasRgb && ($hasSL || $hasWB || $hue !== null)) {
			throw new SassScriptException(
				sprintf(
					"RGB parameters may not be passed along with %s parameters.",
					$hasWB ? "HWB" : "HSL"
				)
			);
		}
	...
	{
		return $this->alterColor($args, "adjust", function (
			$base,
			$alter,
			$max
		) {
	...
	{
		return $this->alterColor($args, "change", function (
			$base,
			$alter,
			$max
		) {
	...
	{
		return $this->alterColor($args, "scale", function (
			$base,
			$scale,
			$max
		) {
	...
	protected function libIeHexStr($args)
	{
		...
		if (\is_null($color)) {
			throw $this->error(
				'Error: argument `$color` of `ie-hex-str($color)` must be a color'
			);
		}
	...
	protected function libMix($args)
	{
		...
		$weightScale =
			$this->assertNumber($weight, "weight")->valueInRange(
				0,
				100,
				"weight"
			) / 100;
		...
		if ($firstAlpha != 1.0 || $secondAlpha != 1.0) {
			$new[] =
				$firstAlpha * $weightScale + $secondAlpha * (1 - $weightScale);
		}
	...
	protected function libHsl($args, $kwargs, $funcName = "hsl")
	{
		...
		$color = $this->toRGB(
			$hueValue,
			max(0, min($saturation->getDimension(), 100)),
			max(0, min($lightness->getDimension(), 100))
		);
...
will be an error in future versions of Sass.\n         on line $line of $fname";
	...
	protected function libRound($args)
	{
		...
		return new Number(
			round($num->getDimension()),
			$num->getNumeratorUnits(),
	...
	protected function libMapRemove($args)
	{
		...
		for ($i = \count($map[1]) - 1; $i >= 0; $i--) {
			if (
				in_array(
					$this->compileStringContent(
						$this->coerceString($map[1][$i])
					),
					$keys
				)
			) {
	 ...
	 * @phpstan-param callable(Number|array): (Number|array) $modify
	 */
	private function modifyNestedMap(
		array $map,
		array $keys,
		callable $modify,
		$addNesting
		...
		$nestedMap =
			$nestedValueIndex !== null
				? $this->tryMap($map[2][$nestedValueIndex])
				: null;
		...
		$newNestedMap = $this->modifyNestedMap(
			$nestedMap,
			$keys,
			$modify,
			$addNesting
	...
	protected function libKeywords($args)
	{
		...
		if (
			...
			throw SassScriptException::forArgument(
				$compiledValue . " is not an argument list.",
				"args"
			);
		}
	...
	protected function libJoin($args)
	{
		...
		switch (
			$this->compileStringContent($this->assertString($sep, "separator"))
		) {
			case "comma":
				$separator = ",";
				break;
	...
	protected function libToLowerCase($args)
	{
		...
		$string[2] = [
			$this->stringTransformAsciiOnly($stringContent, "strtolower"),
		];
	...
	protected function libToUpperCase($args)
	{
		...
		$string[2] = [
			$this->stringTransformAsciiOnly($stringContent, "strtoupper"),
		];
	...
	protected function libRandom($args)
	{
		if (isset($args[0]) && $args[0] !== static::$null) {
			...
			if ($n < 1) {
				throw new SassScriptException(
					"\$limit: Must be greater than 0, was $n."
				);
			}
	 ...
	 * @return array
	 */
	protected function inspectFormatValue(
		$value,
		...
		if ($value[0] === Type::T_LIST) {
			...
			foreach ($value[2] as $k => $listelement) {
				$value[2][$k] = $this->inspectFormatValue(
					$listelement,
					$force_enclosing_display
				);
			}
	 ...
	 * @return array
	 */
	protected function getSelectorArg(
		$arg,
		$varname = null,
		$allowParent = false
	) {
		...
		if (!$this->checkSelectorArgType($arg)) {
			...
			throw SassScriptException::forArgument(
				"$var_value is not a valid selector: it must be a string, a list of strings, or a list of lists of strings",
				$varname
			);
		}
		...
		throw SassScriptException::forArgument(
			"expected more input, invalid selector.",
			$varname
		);
	}
	...
	protected function selectorAppend($selectors)
	{
		...
		while (\count($selectors)) {
			...
			if (!$previousSelectors) {
				throw $this->error(
					"Invalid selector list in selector-append()"
				);
			}
	...
	protected function libSelectorExtend($args)
	{
		...
		$extended = $this->extendOrReplaceSelectors(
			$selectors,
			$extendee,
			$extender
		);
	...
	protected function libSelectorReplace($args)
	{
		...
		$replaced = $this->extendOrReplaceSelectors(
			$selectors,
			$original,
			$replacement,
			true
	 ...
	 * @return array
	 */
	protected function extendOrReplaceSelectors(
		$selectors,
		$extendee,
		$extender,
		$replace = false
	...
	protected function unifyCompoundSelectors($compound1, $compound2)
	{
		...
		while (\count($compound1) || \count($compound2)) {
			...
			if (
				$part1 &&
				($match2 = $this->matchPartInCompound($part1, $compound2))
			) {
				list($compound2, $part2, $after2) = $match2;

				if ($after2) {
					$unifiedSelectors = $this->prependSelectors(
						$unifiedSelectors,
						$after2
					);
				}
				...
				$unifiedSelectors = $this->prependSelectors($unifiedSelectors, [
					$c,
				]);
			...
			if (
				$part2 &&
				($match1 = $this->matchPartInCompound($part2, $compound1))
			) {
	...
	protected function libScssphpGlob($args)
	{
		@trigger_error(
			sprintf(
				'The "scssphp-glob" function is deprecated an will be removed in ScssPhp 2.0. Register your own alternative through "%s::registerFunction',
				__CLASS__
			),
			E_USER_DEPRECATED
		);
		...
		$this->logger->warn(
			'The "scssphp-glob" function is deprecated an will be removed in ScssPhp 2.0.',
			true
		);
