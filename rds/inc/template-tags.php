		...
		$byline = apply_filters(
			...
			sprintf(
				...
				$posted_on
					...
					: esc_html_x("Posted by", "post author", "understrap"),
...
if (!function_exists("understrap_body_attributes")) {
	...
	function understrap_body_attributes()
	{
		 ...
		 */
		$atts = array_unique(
			apply_filters("understrap_body_attributes", $atts = [])
		);
		...
		foreach ($atts as $name => $value) {
			if ($value) {
				$attributes .=
					sanitize_key($name) . '="' . esc_attr($value) . '" ';
			} else {
...
if (!function_exists("understrap_link_pages")) {
	...
	{
		$args = apply_filters("understrap_link_pages_args", [
			"before" =>
				'<div class="page-links">' . esc_html__("Pages:", "understrap"),
			"after" => "</div>",
		]);
		wp_link_pages($args);
	}
