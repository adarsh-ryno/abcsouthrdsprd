...
class Number extends Node implements \ArrayAccess
{
	...
	public function __construct(
		...
		if (is_string($numeratorUnits)) {
			...
			isset(
				$numeratorUnits["numerator_units"],
				$numeratorUnits["denominator_units"]
			)
		) {
	...
	{
		return \count($this->numeratorUnits) === 0 &&
			\count($this->denominatorUnits) === 0;
	}
	...
	{
		return \count($this->numeratorUnits) === 1 &&
			\count($this->denominatorUnits) === 0 &&
			$this->numeratorUnits[0] === $unit;
	}
	...
	public function valueInRange($min, $max, $name = null)
	{
		try {
			...
			throw SassScriptException::forArgument(
				sprintf(
					'Expected %s to be within %s%s and %s%3$s',
					$this,
					$min,
					$this->unitStr(),
					$max
	...
	public function assertNoUnits($varName = null)
	{
		...
		throw SassScriptException::forArgument(
			sprintf("Expected %s to have no units.", $this),
			$varName
		);
	}
	...
	public function assertUnit($unit, $varName = null)
	{
		...
		throw SassScriptException::forArgument(
			sprintf('Expected %s to have unit "%s".', $this, $unit),
			$varName
		);
	}
	...
	{
		return new Number(
			$this->valueInUnits($newNumeratorUnits, $newDenominatorUnits),
			$newNumeratorUnits,
			$newDenominatorUnits
		);
	}
	...
	public function equals(Number $other)
	{
		...
		if ($this->unitless()) {
			return round($this->dimension, self::PRECISION) ==
				round($other->dimension, self::PRECISION);
		}

		try {
			return $this->coerceUnits($other, function ($num1, $num2) {
				return round($num1, self::PRECISION) ==
					round($num2, self::PRECISION);
			});
		} catch (SassScriptException $e) {
			return false;
		}
	 ...
	 */
	private function valueInUnits(
		array $numeratorUnits,
		array $denominatorUnits
	) {
		if (
			$this->unitless() ||
	 ...
	 */
	private function multiplyUnits(
		$value,
		array $numerators1,
		array $denominators1,
		array $numerators2,
		...
		$newDenominators = array_values(
			array_merge($denominators1, $denominators2)
		);
	 ...
	 * @return string
	 */
	private static function getUnitString(
		array $numerators,
		array $denominators
	) {
		if (!\count($numerators)) {
		...
		return implode("*", $numerators) .
			(\count($denominators) ? "/" . implode("*", $denominators) : "");
	}
}
